...
from database import add_user_card_db, delete_user_card_db, get_exact_user_card_db, \
    get_all_cards_or_exact_transactions_db, transfer_money_db
...
async def add_user_card_api(user_id: int, cardholder: str, card_name: str, card_number: int, exp_date: int, otp: int):
    ...
    if otp_test.get(user_id) == otp:
        ...
        result = add_user_card_db(user_id=user_id, cardholder=cardholder, card_name=card_name,
                                  card_number=card_number, exp_date=exp_date, added_date=datetime.now(), balance=0)
    else:
        result = 'Неверный код'
